...
public class QuotedStringTokenizer extends StringTokenizer {
  ...
  public String nextToken() {
    ...
    while (_iPos < len) {
      ch = str.charAt(_iPos);
      if (_iPos == start && delims.indexOf(str.charAt(_iPos)) > -1) start++;
      else if (ch == '"' && bQuotes) {
        if (_iPos + 1 == len || delims.indexOf(str.charAt(_iPos + 1)) > -1)
          return str.substring(start, (_iPos += 2) - 2);
      } else if (delims.indexOf(ch) > -1 && !bQuotes) return str.substring(start, (++_iPos) - 1);
      _iPos++;
    }
    if (_iPos != start) return str.substring(start, _iPos);
