...
public abstract class AbstractAddAccount extends AbstractAction {
  ...
  public AbstractAddAccount(String mnemonic, String account, String handle) {
    ...
    else if (handle.length() > 10) throw new IllegalArgumentException("Handle length > 10");
    else if (handle.length() < 1) throw new IllegalArgumentException("Handle length < 1");
    _sHandle = handle;
  }
